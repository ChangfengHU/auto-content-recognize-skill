...
SKILL_NAME="auto-content-recognize"
WORK_DIR="$(mktemp -d /tmp/auto-content-recognize-skill-XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT
TS="$(date +%Y%m%d%H%M%S)"
RELEASE_PATH="auto-content-recognize/release"
...
"$ROOT_DIR/scripts/upload-file.sh" --file "$ROOT_DIR/skills/auto-content-recognize/scripts/run.sh" --name "${SKILL_NAME}.sh" >/dev/null

echo "SKILL_INSTALL_COMMAND=bash <(curl -fsSL 'https://skill.vyibc.com/install-${SKILL_NAME}.sh?ts=${TS}')"
echo "CLI_COMMAND=bash <(curl -fsSL https://skill.vyibc.com/${SKILL_NAME}.sh) --mode=url --url='https://www.youtube.com/watch?v=Ahfe-BW1cFc' --token='YOUR_TOKEN'"
